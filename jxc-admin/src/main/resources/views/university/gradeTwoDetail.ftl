...
<script>
    ...
    layui.use(['table','layer'],function(){
        ...
        table.render({
            elem: '#test'
            ,url: '/university/selectImageDetailFromTableByGradeTwo'
            ...
            ,cols: [[
                ...
                ,{field:'parentName', title: '标题', sort: true}
                ,{field:'url', title: '缩略图',width: '20%', align: 'center',
                    templet:function (res) {
                        let photograph = '<div lay-event="photograph"><img src='+res.url+'></div>';
                        return photograph;
                    }
                }
                ,{field:'detailUrl', title: '详情图',width: '20%', align: 'center',
                    templet:function (res) {
                        let photograph = '<div lay-event="photograph-detail"><img src='+res.detailUrl+'></div>';
                        return photograph;
                    }
                }
                ,{field:'fileGroup', width:80,title: 'part数', sort: true }
                ,{field:'sort', width:80,title: '排序', sort: true }
                ,{field:'wealth', width:200, title: '操作',
                    templet:function (res){
                        let add = "<button type=\"button\" class=\"layui-btn layui-btn-sm layui-btn-danger\" lay-event=\"add\">新增</button>";
        ...
        table.on('tool(demo)',function (res){
            if(res.event == 'del'){//删除
                ...
                layer.open({
                    ...
                    success: function(layero, index){
                        ...
                        body.find("#name").val(res.data.parentName);
                        ...
                        body.find("#detailPath").val(res.data.detailUrl);
                        body.find("img[id='ID-upload-img-detail']").attr('src',res.data.detailUrl);
                        body.find("img[id='ID-upload-img']").attr('src',res.data.url);
                    }
                });
            }else if(res.event == 'add'){//新增
                layer.open({
                    type:2,
                    title:'添加part图',
                    content:"gradeTwoDetailAdd",
                    area:['640px','580px'],//宽,高
                    end:function (){//关闭之后执行
                        //重载table数据
            ...
            }else if(res.event == 'photograph-detail'){//图片放大
                let t = $(res).find("img");
                if(t == null||t == ''){
                    return;
                }
                layer.open({
                    ...
                    content: '<div style="text-align:center"><img src="' + res.data.detailUrl + '" /></div>'
                });
            }
        });
