    ...
    <div class="layui-container">
        ...
        <div class="layui-row">
            <div class="layui-col-md6">
                <button id = "addImage" type="button" class="layui-btn layui-btn-primary layui-btn-fluid">添加图片作品</button>
            </div>
            <div style="width: 20px"></div>
            <div class="layui-col-md6">
                <button id = "addVideo" type="button" class="layui-btn  layui-btn-primary layui-btn-fluid">添加视频作品</button>
            </div>
...
<script>
    ...
    layui.use(['table','layer'],function(){
        ...
        table.render({
            elem: '#test'
            ,url: '/many/selectImageAndVideoFromTableByWorks'
            ...
            ,cols: [[
                ...
                ,{field:'authorImage', title: '作者头像',
                    templet:function (res) {
                        let photograph = '<div lay-event="photograph-author"><img src='+res.authorImage+'></div>';
                        return photograph;
                    }
                }
                ...
                ,{field:'wealth', width:137, title: '操作',
                    templet:function (res){
                        ...
                        let update = '';
                        if(res.typeFlag == 0){
                            update = "<button type=\"button\" class=\"layui-btn layui-btn-sm layui-btn-danger\" lay-event=\"update-image\">修改</button>";
                        }else{
                            update = "<button type=\"button\" class=\"layui-btn layui-btn-sm layui-btn-danger\" lay-event=\"update-video\">修改</button>";
                        }
        ...
        $("#addImage").click(function (){
            layer.open({
                type:2,
                title:'添加图片作品',
                content:"worksImageAdd",
                area:['640px','640px'],//宽,高
                end:function (){//关闭之后执行
                    //重载table数据
        ...
        table.on('tool(demo)',function (res){
            if(res.event == 'del'){//删除
            ...
            }else if(res.event == 'update-image'){//修改图片
                layer.open({
                    type:2,//iframe弹出层
                    title:'修改图片作品',
                    content:"worksImageUpdate",
                    ...
                    success: function(layero, index){
                        ...
                        body.find("#authorImage").val(res.data.authorImage);
                        body.find("img[id='ID-upload-img-author']").attr('src',res.data.authorImage);;
                        body.find("#path").val(res.data.url);
                        body.find("img[id='ID-upload-img']").attr('src',res.data.url);
                    }
                });
            }else if(res.event == 'update-video'){//修改视频
                layer.open({
                    type:2,//iframe弹出层
                    title:'修改视频作品',
                    content:"worksVideoUpdate",
                    area:['640px','600px'],//宽,高
                    end:function (){//关闭页面执行
                        table.reload("tableId");//刷新table页面
                    },
            ...
            }else if(res.event == 'photograph-author'){//图片放大
                let t = $(res).find("img");
                if(t == null||t == ''){
                    return;
                }
                layer.open({
                    ...
                    content: '<div style="text-align:center"><img src="' + res.data.authorImage + '" /></div>'
                });
            }
        });
